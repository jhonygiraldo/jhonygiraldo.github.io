...
<ol class="learning-objectives">
  <li>For \(\mathbf{A}\in\mathbb{R}^{4\times4}\) and \(\mathbf{x}\in\mathbb{R}^{4}\), what does entry \(i\) of \(\mathbf{A}\mathbf{x}\) equal as a sum? (§2)</li>
  <li>If \(\mathbf{A}\) is a graph's adjacency matrix, what does \((\mathbf{A}^2)_{ij}\) count? (§2)</li>
  <li>What does \(\mathbf{P}\mathbf{A}\mathbf{P}^{\top}\) do, for \(\mathbf{P}\) a permutation matrix? (§3)</li>
  <li>A graph has \(N=10^{6}\) nodes and \(10^{7}\) edges. How much memory does its adjacency matrix need, dense and sparse? (§3)</li>
  <li>State what the spectral theorem guarantees for a real symmetric matrix. (§4)</li>
  <li>\(\mathbf{S}\) is symmetric with eigenvalues in \((-1,1]\). Describe \(\mathbf{S}^{k}\mathbf{x}\) as \(k\to\infty\). (§5)</li>
  <li>What is \(\mathbf{x}^{\top}\mathbf{L}\mathbf{x}\) when \(\mathbf{L}\) is a graph Laplacian, written as a sum over edges? (§4)</li>
  <li>Differentiate \(f(\mathbf{x})=\mathbf{x}^{\top}\mathbf{A}\mathbf{x}\) with respect to \(\mathbf{x}\), for symmetric \(\mathbf{A}\). (§6)</li>
  <li>What is \(\nabla_{\mathbf{Z}}\operatorname{tr}(\mathbf{Z}^{\top}\mathbf{A}\mathbf{Z})\)? (§6)</li>
  <li>If \(\mathbf{U}\in\mathbb{R}^{N\times d}\) and \(\mathbf{V}\in\mathbb{R}^{M\times d}\) with \(d\ll N,M\), what is the maximum rank of \(\mathbf{U}\mathbf{V}^{\top}\), and why does that matter? (§7)</li>
</ol>
```

## The running example

Everything below is illustrated on one graph, so that each new object lands on something familiar. This is the same four-node graph used in Lecture 1.
...
## Vectors, matrices, and what a product means
...
The general statement is that $(\mathbf{A}^{k})_{ij}$ counts walks of length exactly $k$. This is why powers of $\mathbf{A}$ describe reach on a graph, and it is the seed of the diffusion sequence in Lecture 2.
:::
...
For a graph this is not a technical detail: **$\mathbf{A}$ is symmetric exactly when the graph is undirected.** Lecture 1's directed example has $A_{13}=1$ but $A_{31}=0$, and §4's spectral guarantees will apply only to the symmetric case.

::: {.checkpoint}
**Where this is used**

$\mathbf{A}\mathbf{x}$ as message passing is Lecture 2 §7 and every model after it. $(\mathbf{A}^{2})_{ij}$ as a walk count is Lecture 1's worked example and Exercise 1. Inner-product scoring is Lecture 1's decoder and Lecture 5's $\operatorname{score}(u,v)=\langle\mathbf{u},\mathbf{v}\rangle$. Shape tracking is Lecture 2's two-layer GCN example.
:::

## Structured and sparse matrices
...
Left-multiplying scales rows, right-multiplying scales columns, so the entry picks up both factors. This is the **symmetric normalization**, and knowing it is a two-sided scaling explains a property that catches people out: it is *not* row-stochastic. Its rows do not sum to one. (The row-stochastic operator is $\mathbf{D}^{-1}\mathbf{A}$, the random-walk matrix.)
...
A matrix is **sparse** when almost all entries are zero. Real graphs are extremely sparse: a social network with $10^{6}$ users and $100$ friends each has $10^{8}$ nonzeros out of $10^{12}$ entries — one in ten thousand.
...
| coordinate / `edge_index` | one $(i,j)$ pair per edge | $O(M)$ |
| compressed sparse row | values plus row offsets | $O(N+M)$ |

::: {.worked-example}
**Worked example — why dense storage fails**

Take $N=10^{6}$ nodes with $100$ neighbors each, so $M=5\times10^{7}$ undirected edges, at 8 bytes per number.

**Dense:** $8N^{2}=8\times10^{12}$ bytes $=8$ terabytes.

**Edge list:** two indices per stored direction, $8\times2\times2M=1.6\times10^{9}$ bytes $=1.6$ gigabytes.
...
The cost of a *product* changes too. Dense $\mathbf{A}\mathbf{x}$ costs $O(N^{2})$ multiply–adds; sparse costs $O(M)$, since zero entries contribute nothing. This is the origin of the $O(|\mathcal{E}|)$ complexity quoted throughout the course.
...
which costs $O(kM)$ and never stores anything denser than $\mathbf{A}$.
:::

::: {.checkpoint}
**Where this is used**

Symmetric normalization is the GCN operator of Lecture 2 and the LightGCN operator of Lecture 5. Permutation equivariance is Lecture 1 §6 and the justification for graph convolution in Lecture 2. Sparse storage and the $O(N^{2})$-versus-$O(N+M)$ contrast are Lecture 1 §4 and Lecture 3's memory-wall example. The "never form $\mathbf{S}^{k}$" rule is Lecture 2's Check 2.
:::
...
| $\vert\lambda\vert>1$ | $\to\infty$ | **blows up** |
...
**Over-smoothing** (Lecture 2). For the GCN operator, $\lambda_{\max}=1$ with a smooth eigenvector and everything else has $|\lambda|<1$. Stack many layers and every component except the smoothest is erased — all node embeddings converge to the same thing, and the model can no longer tell nodes apart.

**Low-pass filtering** (Lectures 2 and 4). The same statement from the other side: components at low graph frequency survive, components at high frequency die. Repeated propagation *is* a low-pass filter. Note the scope — this is a statement about repeated *fixed linear* propagation, not about a trained network with weights and nonlinearities.
...
By $k=20$ the signal is essentially a multiple of $\mathbf{u}_1$: three of its four degrees of freedom are gone. Nothing was lost to rounding — this is what the operator *does*.

Now the important question: how fast? The decay of component $i$ relative to the survivor is $(\lambda_i/\lambda_1)^{k}$, so the **ratio** $\lambda_2/\lambda_1$ sets the timescale. Here $0.9/1=0.9$, decaying slowly; had $\lambda_2$ been $0.3$ the collapse would have taken a handful of steps. This ratio is called the **spectral gap**, and a graph with a small gap tolerates more layers than one with a large gap.
:::
...
### Two constructions for Lecture 4 only
...
You need this once: to write the Hessian of Lecture 4's reconstruction objective as a single matrix and ask whether it is invertible. If you are not reading Lecture 4, skip it.
...
$f(n)=O(g(n))$ means $f$ grows no faster than $g$ up to a constant: $O(N+M)$ storage and $O(M)$ per sparse product. Constants and lower-order terms are dropped, which is a feature when comparing $N^{2}$ against $N$ and a trap when comparing two $O(M)$ methods whose constants differ by a hundred.
...
- **Isolated nodes.** Degree zero means no outgoing transition, clustering coefficient defined as $0$, and $1/0=0$ in degree-normalized operators.
...
Compute $\mathbf{A}^{3}$ for the running graph. Interpret $(\mathbf{A}^{3})_{11}$ and explain why it is nonzero even though there is no triangle through node 1.
...
Standard references for anything here in more depth: Strang's *Linear Algebra and Learning from Data* for the matrix material, and Boyd and Vandenberghe's *Convex Optimization* for §6 onward. Ortega's *Introduction to Graph Signal Processing* [@ortega2022introduction] is the natural continuation of §4 and §5 for readers who want the spectral graph theory properly.
