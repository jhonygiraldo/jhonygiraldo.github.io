...
Now set the likelihood up properly, because the obvious guess is wrong. GraphRNN models a *sequence*: it defines $p_\theta(s)$, whereas we want $p_\theta(\mathcal{G})$. The two are related by summing over the **distinct sequences that decode to that graph**,

$$
p_\theta(\mathcal{G})=\sum_{s\,:\,f(s)=\mathcal{G}}p_\theta(s),
$$

which is a *sum of probabilities*, not an average of them weighted by an ordering sampler [@you2018graphrnn]. Note also that different permutations can produce the **same** sequence, through the graph's automorphisms, so the map from orderings to sequences is not injective.

Training instead maximizes $\mathbb{E}_{q}[\log p_\theta(s)]$ for the sampler $q$ over orderings. Use Jensen's inequality on

$$
\log p_\theta(\mathcal{G})=\log\mathbb{E}_{q}\!\left[\frac{p_\theta(s)}{q(s\mid\mathcal{G})}\right]
$$

to show that $\mathbb{E}_{q}[\log p_\theta(s)-\log q(s\mid\mathcal{G})]$ is a **lower bound** on $\log p_\theta(\mathcal{G})$, and state when it is tight. Why does the entropy term not affect the maximizer when $q$ is fixed?

Then make it concrete on a three-node path. Enumerate its $3!=6$ orderings, identify which produce the same sequence (the path has a nontrivial automorphism), and count the **distinct** sequences. Assign each a probability under a model of your choosing and compute both sides of the inequality. Finally, explain what the BFS restriction does to the support of $q$ and why that helps.
